...
namespace {
// Milliseconds since boot. Replaces the factory HAL's now_ms() so this
// servo layer is decoupled from the StackChan HAL singleton.
...
void Servo::update()
{
    ...
    // Auto release torque on rest
        ...
        if (now_ms() - _last_torque_check_tick > 200) {
            if (getTorqueEnabled()) {
                setTorqueEnabled(false);
            }
            _last_torque_check_tick = now_ms();
        }
    }
}
