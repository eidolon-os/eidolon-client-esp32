...
private:
    ...
    void InitializeCamera() {
        // OV5640 reset/power-down are wired through the XL9555 I/O expander, not
        // dedicated SoC pins, so drive the reset sequence here before esp_video
        // brings up the sensor (its reset_pin/pwdn_pin stay GPIO_NUM_NC).
        ...
        esp_cam_ctlr_dvp_pin_config_t dvp_pin_config = {
            ...
            // CAM_PIN_XCLK is GPIO_NUM_NC: the module has its own oscillator, so
            // no clock is driven, but esp_video still takes the frequency value.
            .xclk_io = CAM_PIN_XCLK,
        };

        // OV5640 SCCB is on dedicated pins (GPIO38/39), separate from the audio
        // codec + XL9555 bus (I2C_NUM_0). Have esp_video bring up a fresh SCCB
        // master on I2C_NUM_1 rather than reusing i2c_bus_.
        ...
        esp_video_init_dvp_config_t dvp_config = {
            ...
            .reset_pin = CAM_PIN_RESET,  // GPIO_NUM_NC (reset via XL9555 above)
            .pwdn_pin = CAM_PIN_PWDN,    // GPIO_NUM_NC (pwdn via XL9555 above)
