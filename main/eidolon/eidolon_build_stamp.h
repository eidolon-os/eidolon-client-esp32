...
// LiveKit SDK version at build time; this header includes it when present and
// otherwise falls back to placeholders so a plain `idf.py build` (no toolkit)
// still compiles. The app prints these once at boot as one
// "EIDOLON-BUILDSTAMP ..." line so you can confirm exactly which firmware +
// SDK the device is actually running (PROJECT_VER alone is a static "1.0.0").
