...
def write_overlay(
    ...
    for item in board.sdkconfig_append:
        ...
        if key in {
            ...
            continue
        if aec_afe_mode != "board" and key in {
            "CONFIG_EIDOLON_DEVICE_AEC_AFE_MODE_LOW_COST",
            "CONFIG_EIDOLON_DEVICE_AEC_AFE_MODE_HIGH_PERF",
        }:
            continue
        lines.append(item)
    if aec_afe_mode == "low_cost":
        lines.extend(
            [
    ...
    elif aec_afe_mode == "high_perf":
        lines.extend(
            [
                "# CONFIG_EIDOLON_DEVICE_AEC_AFE_MODE_LOW_COST is not set",
...
def rms_from_energy(energy: int | float, samples: int) -> float:
    if samples <= 0:
        return 0.0
    return math.sqrt(float(energy) / samples)
...
def analyze(
    ...
    for case_id, streams in audio.items():
        ...
        raw_frames = int(stats.get("raw_frames", 0)) if isinstance(stats, dict) else 0
        aec_frames = int(stats.get("aec_frames", 0)) if isinstance(stats, dict) else 0
        if raw_frames > 0 or aec_frames > 0:
            raw_rms = rms_from_energy(int(stats.get("raw_energy", 0)), raw_frames * 320)
            aec_rms = rms_from_energy(int(stats.get("aec_energy", 0)), aec_frames * 320)
            ref_rms = rms_from_energy(int(stats.get("ref_energy", 0)), raw_frames * 320)
            metric_source = "device_full_case_energy"
        else:
            raw_rms = preview_raw_rms
            aec_rms = preview_aec_rms
            ref_rms = preview_ref_rms
            metric_source = "serial_preview_pcm"
        ...
        metrics[case_id] = {
            ...
            "erle_db": erle,
            "preview_raw_rms": preview_raw_rms,
            "preview_raw_dbfs": dbfs(preview_raw_rms),
            "preview_reference_rms": preview_ref_rms,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="board",
        help="Override the device-side AEC AFE mode for this qualification build only",
    )
